...
Hard constraints:
  ...
  - Default to current-major ecosystem versions unless a specific constraint forces otherwise. Examples: ESLint 9 flat config (not 8 legacy .eslintrc) on Node 20+, Husky 9 (not 8), Prettier 3 (not 2). If a package has reached a stable major release and is broadly adopted, choose it. If a constraint forces an older major, state the constraint explicitly in the commit body.
  - If a task scaffolds developer tooling that runs on commit, include lint-staged (or equivalent staged-file runner) by default. Pre-commit hooks that run against the whole repo are acceptable for a one-file repo but do not scale; wire the staged-file runner at scaffolding time, not later.
